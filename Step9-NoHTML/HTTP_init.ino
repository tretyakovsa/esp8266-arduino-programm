...
void HTTP_init(void) {
  ...
  HTTP.on("/config.live.json", handle_ConfigJSON); // формирование configs.json страницы для передачи данных в web интерфейс
  // API для устройства
  ...
  HTTP.on("/analog.json", analogJson); //
...
String analogJson(){
  String a = "{\"data\":\[";
  a += analogRead(A0);
  a +=  "],\"points\":10,\"refresh\":3000,\"options\":\"low:0\"}";
  HTTP.send(200, "text/plain", a); // отправляем ответ о выполнении
  }
